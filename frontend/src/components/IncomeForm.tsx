...
export const IncomeForm = ({ onSuccess }: IncomeFormProps) => {
  ...
  const { data: categories = [] } = useQuery<Category[]>({
    queryKey: ["categories", "income", clientId],
    queryFn: async () => {
      const res = await api.get("/categories", {
        params: { type: "income", clientId },
      });
      return res.data;
    },
    enabled: !!clientId,
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await notifySuccess("Ingreso guardado");
      setTitle("");
      setAmount("");
      setCategoryId("");
      onSuccess?.();
    ...
    onError: async () => {
      await notifyError("No se pudo guardar el ingreso");
    },
  });
